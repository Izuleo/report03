...
public class CList <E>{
	private CNode last; //리스트의 마지막 노드(항목)을 가리킨다
    ...
    CNode newNode = new CNode(newItem, null); //새 노드 생성
	...
	public CNode delete() {//last가 가리키는 노드의 다음 노드를 제거
		if(isEmpty())
			throw new NoSuchElementException();
		CNode x =last.getNext();  //x가 리스트의 첫 노드를 가리킴
		if(x == last)
